...
def detect_pdf_type(pdf_path):
    """Detects PDF type: 'text_pdf' (digital) or 'image_pdf' (scanned) using the first page."""
...
def extract_with_camelot(pdf_path):
    """Extract the first table from a digital PDF using Camelot (lattice first, then stream)."""
    import camelot
...
def ocr_extract(pdf_path):
    """Extracts test tables from a scanned (image) PDF using OCR and regex."""
    ...
    for line in lines:
        ...
        # Test name but result is on next line
    ...
    # Remove short or invalid test names
...
def save_table_outputs(df, pdf_path):
    """
    Save the extracted DataFrame as both CSV and JSON next to the original PDF.
    """
...
#     import sys
#     pdf_path = sys.argv[1] if len(sys.argv) > 1 else "sample_tables_medical.pdf"
