...
def extract_pdf_text_smart(
    ...
    - Uses direct text extraction (PyMuPDF) when possible
    - Falls back to OCR (pdf2image + pytesseract) for pages that look scanned
      or when force_ocr=True.

    Args:
        pdf_path: str or Path to the PDF
        force_ocr: if True, use OCR for all pages
        scanned_threshold: if direct char_count < this, treat page as scanned
        dpi: DPI for pdf2image when doing OCR
