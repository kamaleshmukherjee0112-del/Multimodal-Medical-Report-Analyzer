...
Fallback parser for LAB reports when no real table is detected.
Reconstructs structured lab rows from visually-tabular raw text.

Design principles:
- LAB ONLY (never used for imaging / ECG)
...
def build_lab_rows_from_text(raw_text: str) -> List[Dict]:
    """
    Extract structured lab rows from raw lab report text.

    Returns a list of dicts compatible with test_results.from_table
    """
    if not raw_text:
        return []
...
# ------------------------------------------------------------
# Block extraction helpers (LINE-BASED)
# ------------------------------------------------------------
